...
public function details(int $id, EntityManagerInterface $entityManager): Response
{
    ...
    return $this->render('client/details.html.twig', [
        'client' => $client,
        'dettes' => $dettes, // Ajoutez cette ligne pour passer les dettes à la vue
...
public function addClient(Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    // Si une dette est spécifiée, créer la dette associée
    if (!empty($data['detteMontant']) && !empty($data['detteDate'])) {
        ...
        $dette->setClient($client); // Lier la dette au client
        $dette->setMontantVerser(0); // Par défaut aucun montant versé
        $dette->setStatut('non_solde'); // Par défaut la dette n'est pas soldée
