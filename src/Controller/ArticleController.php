...
class ArticleController extends AbstractController
{
    ...
    {
        // Recherche des articles via les filtres
...
public function saveSelection(Request $request, EntityManagerInterface $entityManager): JsonResponse
{
    ...
    try {
        $data = json_decode($request->getContent(), true, 512, JSON_THROW_ON_ERROR); // Décodage JSON avec gestion des erreurs
    ...
    foreach ($data as $item) {
        ...
        // Diminuez le stock de l'article
...
{
    // Récupération des articles avec stock > 0
    $articlesDisponibles = $entityManager->getRepository(Article::class)
        ->createQueryBuilder('a')
        ->where('a.qteStock > 0') // Seuls les articles en stock
        ->getQuery()
        ->getResult();

    // Vérifiez que la liste des articles n'est pas vide
    ...
    $form = $this->createForm(ApprovisionnementType::class, null, [
        'articles' => $articlesDisponibles, // Passe les articles disponibles à l'option
    ]);

    return $this->render('approvisionnement/new.html.twig', [
        'form' => $form->createView(),
        'articles' => $articlesDisponibles, // Passe les articles au template si besoin
    ]);
}
